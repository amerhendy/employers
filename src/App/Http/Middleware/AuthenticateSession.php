...
namespace Amerhendy\Employers\App\Http\Middleware;
...
class AuthenticateSession extends LaravelAuthenticateSession
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if (! $request->session()->has('password_hash_'.Amer_guard_name('Employer'))) {
            $this->storePasswordHashInSession($request);
        }

        if ($request->session()->get('password_hash_'.Amer_guard_name('Employer')) !== $this->user->getAuthPassword()) {
            $this->logout($request);
        }
    ...
    protected function storePasswordHashInSession($request)
    {
        ...
        $request->session()->put([
            'password_hash_'.Amer_guard_name() => $this->user->getAuthPassword(),
        ]);
    }
    ...
    protected function logout($request)
    {
        ...
        throw new AuthenticationException('Unauthenticated.', [Amer_guard_name()], Amerurl('login'));
    }
