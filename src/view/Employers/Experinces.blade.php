...
if(isset($field['value'])){
    ...
    $field['subfields']['Mosama_JobNames']['value']=$field['value'][1];
}
...
<div "class"="form-group col-sm-12" "data-entity"="jobnames" "data-init-function"="bpFieldInitChecklistDependencyElement">
    <div class="row">
        ...
        <div class="col-sm-6"  id="fullDiv_Mosama_JobNames">
            @include(fieldview('select2_from_ajax'),$Mosama_JobNames)
        </div>
    </div>
</div>
...
<script>
    ...
    $('select[name^=Mosama_Groups]').on("select2:select", function() {
        ...
        attatchlink('Mosama_JobNames',{'Mosama_Groups':value});
    });
    @if(isset($field['value']))
    setold();
    @endif
    function setold(){
        attatchlink('Mosama_JobNames',{'Mosama_Groups':$('select[name^=Mosama_Groups]').val()});
